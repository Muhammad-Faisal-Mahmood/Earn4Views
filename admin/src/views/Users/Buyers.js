...
const Buyer = () => {
  ...
  const FetchBuyers = async () => {
      try {
          const response = await fetch(`${BaseURL}/BuyersList`, {
              headers: {
  ...
  const DeleteBuyers = async (id) => {
      try {
          const response = await fetch(`${BaseURL}/deleteBuyer/${id}/`, {
              method: 'DELETE',
              headers: {
                  'AdminODSToken': sessionStorage.getItem('AdminODSToken'),
              }
          ...
          if (data.success) {
              showAlert('Buyer Deleted', 'success');
              FetchBuyers()
          } else {
      ...
      <CRow>
          <CCol xs>
              <CCard className="mb-4">
                  <CCardHeader>Buyers List</CCardHeader>
                  <CCardBody>
