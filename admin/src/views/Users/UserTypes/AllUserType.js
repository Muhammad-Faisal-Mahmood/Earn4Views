...
const AllUserType = () => {
    ...
    const DeleteFunction = async (id) => {
        try {
            const response = await fetch(`https://stealth-ecommerce-12f9ab59b8a8.herokuapp.com/users/user_type/${id}/`, {
                method: 'DELETE',
                headers: {
            ...
            if (response.ok) {
                ...
                showAlert('User Type Deleted successfully','success');
                FetchUser();
            ...
            <CTable align="middle" className="mb-0 border" hover responsive>
                <CTableHead color="light">
                    <CTableRow>
                        ...
                        <CTableHeaderCell>Discount Allowed</CTableHeaderCell>
                ...
                <CTableBody>
                    {UsersTypes.map((item, index) => (
                        <CTableRow v-for="item in tableItems" key={index}>
                            ...
                            <CTableDataCell>
                                <div>{item.discount_allowed}</div>
                            </CTableDataCell>
                            <CTableDataCell>
                                <div className="col">
                                    <CButton color="primary mx-2"
                                    onClick={()=>{
                                        navigate("/users/editusertype", { state: item });
                                      }}>
                                        Edit
                                        </CButton>
